...
describe('admin.controller.js', function () {
  ...
  describe('GET /inventories/:inventory/history', function () {
    it('should return a list of inventory changes', async function () {
      const card = await Card.findOne({number: '5'}).populate('inventory');
      card.inventory.orderNumber = '123';
      await card.inventory.save();

      return await test.request
        .get(`/api/admin/inventories/${card.inventory._id}/history`)
        .set('Authorization', `bearer ${test.tokens.admin1.token}`)
        .then(res => {
          const data = res.body;
          expect(data).to.be.an('array');
  ...
  describe('POST /inventories/:inventory/revert/:log', function () {
    it('should revert an inventory to one of its previous state', async function () {
      const card = await Card.findOne({number: '5'});

      const logId = await test.request
        .get(`/api/admin/inventories/${card.inventory}/history`)
        .set('Authorization', `bearer ${test.tokens.admin1.token}`)
        .then(res => {
          const data = res.body;
          expect(data).to.be.an('array');
      ...
      return await test.request
        .post(`/api/admin/inventories/${card.inventory}/revert/${logId}`)
        .set('Authorization', `bearer ${test.tokens.admin1.token}`)
        .then(async res => {
          expect(res).to.have.status(200);
          const updatedInventory = await Inventory.findById(card.inventory);
          expect(updatedInventory.orderNumber).to.not.equal('123');
        });
    });
  });
});
