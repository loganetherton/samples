...
import {determineSellTo} from './card.helpers';
...
describe('card.controller.js', function () {
  ...
  describe('POST api/cards/newCard', function () {
    ...
    it('should set lqCustomerName when specified', async function () {

    });

    it('should populate both the card and inventory with the correct userTime', async function () {
  ...
  describe('GET api/cards/:customerId/receipt', function () {
    it('should return all cards that have not been reconciled', async function () {
      const customerId = test.getDefaultReferenceId('customers');

      const response = await test.getExistingCardsReceipt();
      ...
      const reconciledResponse = await test.getExistingCardsReceipt();

      expect(reconciledResponse).to.have.status(200);
      expect(reconciledResponse.body.data).to.have.length(0);
    });
  });

  describe('POST api/cards/modify', function () {
    it('should remove cards from liquidation', async function () {
      const inventory = await Inventory.findOne({customer: test.getDefaultReferenceId('customers')});

      const response = await test.modifyInventory(inventory._id, 'notAddedToLiquidation');

      expect(response).to.have.status(200);
      expect(response.body.soldToLiquidation).to.be.equal(false);
    });

    it('should add cards to liquidation', async function () {
      const inventory = await Inventory.findOne({customer: test.getDefaultReferenceId('customers')});

      const response = await test.modifyInventory(inventory._id, 'soldToLiquidation');

      expect(response).to.have.status(200);
      expect(response.body.soldToLiquidation).to.be.equal(true);
    });
  });

  describe('POST api/cards/edit/balance', function () {
    it('should edit the balance of a card', async function () {
      const inventory = await Inventory.findOne({});

      const response = await test.request.post('/api/card/edit/balance')
      .set('Authorization', 'bearer ' + test.tokens.admin1.token)
      .send({cardId: {_id: inventory._id}, balance: 30});

      expect(response).to.have.status(200);

      const updatedInventory = await Inventory.findOne({_id: inventory._id}).populate('card');

      expect(updatedInventory.balance).to.be.equal(30);
      expect(updatedInventory.card.balance).to.be.equal(30);
    });

    it('should edit the balance of multiple cards', async function () {
      const inventories = await Inventory.find({}).limit(2);

      const response = await test.request.post('/api/card/edit/balance')
      .set('Authorization', 'bearer ' + test.tokens.admin1.token)
      .send({ids: inventories.map(inv => inv._id), balance: 75});

      expect(response).to.have.status(200);

      for (const inv of inventories) {
        const updatedInventory = await Inventory.findOne({_id: inv._id}).populate('card');

        expect(updatedInventory.balance).to.be.equal(75);
        expect(updatedInventory.card.balance).to.be.equal(75);
      }
    });
  });

  describe('POST api/cards/edit/setCardValue', function () {
    it('should set activityStatus to sentToSmp', async function () {
      const inventory = await Inventory.findOne({});

      const response = await test.request.post('/api/card/edit/setCardValue')
      .set('Authorization', 'bearer ' + test.tokens.admin1.token)
      .send({inventoryId: inventory._id, type: 'activityStatus', status: 'sentToSmp'});

      expect(response).to.have.status(200);

      const updatedInventory = await Inventory.findOne({});
      expect(updatedInventory.activityStatus).to.be.equal('sentToSmp');
    });

    it('should set verifiedBalance', async function () {
      const inventory = await Inventory.findOne({});

      const response = await test.request.post('/api/card/edit/setCardValue')
      .set('Authorization', 'bearer ' + test.tokens.admin1.token)
      .send({inventoryId: inventory._id, type: 'verifiedBalance', status: 39});

      expect(response).to.have.status(200);

      const updatedInventory = await Inventory.findOne({});
      expect(updatedInventory.verifiedBalance).to.be.equal(39);
    });
  });

  describe('POST api/cards/massUpdate', function () {
    it('should mass update multiple cards', async function () {
      const inventories = await Inventory.find({}).limit(2);

      const response = await test.request.post('/api/card/massUpdate')
      .set('Authorization', 'bearer ' + test.tokens.admin1.token)
      .send({ids: inventories.map(i => i._id), values: {cqAch: '4897548'}});

      expect(response).to.have.status(200);

      const updatedInventories = await Inventory.find({_id: {$in: inventories.map(i => i._id)}});

      for (const inv of updatedInventories) {
        expect(inv.cqAch).to.be.equal('4897548');
      }
    });
  });

  describe('POST api/cards/reject', function () {
    it('should reject cards when the verified balance is lower than the claimed balance', async function () {
      const inventory = await Inventory.findOne({});
      inventory.verifiedBalance = 0;
      await inventory.save();

      const response = await test.rejectCard([inventory._id]);

      expect(response).to.have.status(200);

      const updatedInventory = await Inventory.findById(inventory._id);

      expect(updatedInventory.rejected).to.be.equal(true);
      expect(updatedInventory.credited).to.be.equal(false);
    });

    it('should credit cards when the verified balance is higher than the claimed balance', async function () {
      const inventory = await Inventory.findOne({});
      inventory.verifiedBalance = 1000;
      await inventory.save();
      ...
      expect(updatedInventory.credited).to.be.equal(true);
      expect(updatedInventory.rejected).to.be.equal(false);
    });
  });

  describe('determineSellTo', function () {
    let retailer;
    let company1;
    let settings1;
    let company3;
    let settings3;
    before(async function () {
      await test.createCompanyAndCorporateAdminUser(3, {}, {}, {disableLimits: true});
      retailer = await Retailer.findOne();
      company1 = await Company.findOne({name: 'Test1'});
      settings1 = await company1.getSettings();
      company3 = await Company.findOne({name: 'Test3'});
      settings3 = await company3.getSettings();
    });
    it('should sell to the highest SMP when there is more than one not in violation of sell limits', async function () {
      const res = determineSellTo(retailer, 50, settings1);
      expect(res.rate).to.be.equal(0.9);
      expect(res.smp.toLowerCase()).to.be.equal('cardcash');
      expect(res.type.toLowerCase()).to.be.equal('electronic');
    });

    /**
     * @todo The switching system we have right now isn't guaranted to result in alternating results
     * and isn't really testable
     */
    it('should alternate SMPs when there is a rate match', async function () {
      // const matchedRetailer = Object.assign({}, retailer);
      // matchedRetailer.sellRates.cardCash = 0.8;
      // const res = determineSellTo(matchedRetailer, 50, settings1);
      // expect(res.rate).to.be.equal(0.8);
      // expect(res.smp.toLowerCase()).to.be.equal('cardcash');
      // expect(res.type.toLowerCase()).to.be.equal('electronic');
    });

    it('should reject a card when it violates all sell limits and disableLimits is false', async function () {
      const res = determineSellTo(retailer, 500, settings1);
      expect(res).to.be.equal(false);
    });

    it('should accept a card and assign to CardQuiry as SMP when it violates all sell limits and disableLimits is true',
      async function () {
        const res = determineSellTo(retailer, 500, settings3);
        expect(res.rate).to.be.equal(1);
        expect(res.smp.toLowerCase()).to.be.equal('cardquiry');
        expect(res.type.toLowerCase()).to.be.equal('electronic');
      });
  });
});
