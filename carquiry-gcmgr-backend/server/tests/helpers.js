...
export default class TestHelper extends Requests {
  ...
  resetCredentials() {
    return {
      ...
      corporateAdmin3: {
        email: 'corporateadmin3@test.com',
        password: 'test'
      },
      manager1: {
  ...
  getDefaultReferenceId(type, setNumber = 1) {
    if (!type) {
      ...
      throw new Error('Unable to determine reference type');
    }
  ...
  /**
   * Get most recently creatly BI request log
   * @returns {Promise<IBiRequestLog>}
   */
  async getMostRecentBiRequestLog() {
    const logs = await BiRequestLog.find().sort({created: -1}).limit(1);
    return logs[0];
  }

  /**
   * Check to make sure callbacks are well formed
   * @param callback
   * @param number
   * @param pin
   * @param statusCode
   * @param lqCustomerName
   * @param callbackType
   * @returns {Promise<void>}
   */
  async checkCallbacks(callback, number, pin, statusCode, lqCustomerName = null, callbackType = 'biComplete') {
    const hasStatusCode = typeof statusCode === 'number';
    let status = null;
    if (hasStatusCode) {
      status = config.callbackStatus[statusCode];
    }
    expect(callback).to.have.property('number');
    expect(callback.number).to.be.equal(number);
    expect(callback).to.have.property('pin');
    expect(callback.pin).to.be.equal(pin);
    expect(callback).to.have.property('verifiedBalance');
    if (hasStatusCode) {
      expect(callback).to.have.property('statusCode');
      expect(callback.statusCode).to.be.equal(statusCode);
    } else {
      expect(callback.statusCode).to.be.undefined;
    }
    if (lqCustomerName) {
      expect(callback).to.have.property('lqCustomerName');
      expect(callback.lqCustomerName).to.be.equal(lqCustomerName);
    } else {
      expect(callback.lqCustomerName).to.be.undefined;
    }
    expect(callback).to.have.property('body');
    const body = callback.body;
    expect(body).to.have.property('number');
    expect(body.number).to.be.equal(number);
    expect(body).to.have.property('pin');
    expect(body.pin).to.be.equal(pin);
    expect(body).to.have.property('callbackType');
    expect(body.callbackType).to.be.equal(callbackType);
    expect(body).to.have.property('retailer');
    if (hasStatusCode) {
      expect(body).to.have.property('statusCode');
      expect(body.statusCode).to.be.equal(statusCode);
      expect(body).to.have.property('status');
      expect(body.status).to.be.equal(status);
    } else {
      expect(body.statusCode).to.be.undefined;
    }
    if (lqCustomerName) {
      expect(body).to.have.property('lqCustomerName');
      expect(body.lqCustomerName).to.be.equal(lqCustomerName);
    } else {
      expect(body.lqCustomerName).to.be.undefined;
    }
  }

  /**
   * Create and then complete a BiRequestLog
   * @param number
   * @param balance
   * @param invalid
   * @param lqCustomerName
   * @param prefix
   * @param callbackUrl
   * @param userIndex Index of user making request
   * @returns {Promise<void>}
   */
  async createAndCompleteBiLog(number, balance, invalid = false, lqCustomerName = null, prefix = null, callbackUrl = null, userIndex = 1) {
    const biLogData = {
      number,
      pin     : number,
      autoSell: true,
    ...
    if (invalid) {
      biLogData.invalid = invalid;
    }
    if (lqCustomerName) {
      biLogData.lqCustomerName = lqCustomerName;
    }
    if (prefix) {
      biLogData.prefix = prefix;
    }
    if (callbackUrl) {
      biLogData.callbackUrl = callbackUrl;
    }
    await this.createBiLog(biLogData, true, userIndex, userIndex);
    const log = await this.getMostRecentBiRequestLog();
    return await this.completeBiLog({
      number,
      pin: number,
      balance,
      invalid
